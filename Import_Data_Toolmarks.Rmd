...
```{r functions_to_plot}
Extract_One_profile <- function(selection, number){
  a <- as.data.frame(cbind(x=unlist(selection[number,]$x),
           y= unlist(selection[number,]$y)))
  comment(a) <- selection[number,]$File
  return(a)
}
...
b <- Extract_One_profile(Selection1,5)
